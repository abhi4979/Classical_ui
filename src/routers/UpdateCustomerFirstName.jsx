...
export default function UpdateCustomerFirstName() {
 ...
 const [customerNumber, setCustomerNumber] = useState('');
  ...
  const onSubmit = async (formData) => {
   updateCustomer({
     url: `http://localhost:8099/api/v1/customer/updateFirstName/${customerNumber}/${formData.contactFirstName}`,
         data: formData,
         onSuccess: ()=> {
  ...
  return (
    <div>
      <div className="p-6 max-w-2xl mx-auto bg-white rounded shadow">
        <h2 className="text-2xl font-semibold mb-4">Update Customer First Name</h2>
      ...
      { isFetched &&
        <form onSubmit={handleSubmit(onSubmit)} className='space-y-4'>
          <div>
            <label className="block font-medium mb-1">Customers First Name</label>
            <input
              type="text"
            ...
            {errors.contactFirstName && (
              <p className="text-red-600 text-sm">{errors.contactFirstName.message}</p>
            )}
          </div>
