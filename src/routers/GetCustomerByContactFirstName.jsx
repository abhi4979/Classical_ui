...
export default function GetCustomerByContactFirstName() {
  ...
  const fetchCustomers = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`http://localhost:8099/api/v1/customer/customer_firstname/${firstname}`);
      ...
      alert("Error fetching customers by first name.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="p-6 bg-gray-100 min-h-screen">
      <h2 className="text-2xl font-bold mb-4">Search Customers by First Name</h2>
      ...
      {loading ? (
            ...
            <div key={cust.customerNumber} className="bg-white rounded-xl shadow p-4">
              ...
              {cust.salesRepEmployeeNumber && (
                <div className="mt-3 border-t pt-2 text-sm text-gray-700">
                  ...
                  <p>
                    🏢 Office: {cust.salesRepEmployeeNumber.office?.city}, {cust.salesRepEmployeeNumber.office?.state}
                  </p>
                </div>
              )}
            </div>
        ...
        firstname && <p className="text-gray-600 mt-4">No customers found.</p>
      )}
    </div>
  );
}
